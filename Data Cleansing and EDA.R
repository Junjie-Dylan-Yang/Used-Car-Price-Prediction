...
data = subset(data_og,select = -c(region_url,county,lat,long, VIN, 
                                  image_url, image_url, description, posting_date,region))
...
data = data%>%filter(data$odometer >= 50)
...
#!!!!!!!!!!!!!!!!!!!!!!!Do not run below until EDA was finished!!!!!!!!!!!!!!!!!
...
data = data%>%
  mutate(brand = ifelse(data$manufacturer %in% luxury, "luxury","common"))
